...
data_4_voronoid<-BALA_all_data %>% 
  ...
  summarise(weight = length(unique(MF)))
...
temptax<-temptax[order(temptax$order), ]
...
ggsave(filename = "../Plots/richness_voron_maps_updated.pdf", plot_2_save, width = 15, height = 10)
ggsave(filename = "../Plots/richness_voron_maps_updated.png", plot_2_save, width = 15, height = 10)
